...
### Panoramica generale sui sistemi embedded, classi di device, caratteristiche, ordini di grandezza (dimensioni, potenze, consumi, ecc.) e ambiti di utilizzo

Il numero di dispositivi che rientra nell'ambito embedded sono molti e assai diversificati tra di loro, sia in termini hardware che in termini di implementazioni software. Alcuni settori di utilizzo dei sistemi embedded sono:
...
- smart TV, telefoni, decoder, media player
- IoT (Internet of Things)

La potenza computazionale di un sistema embedded è un fattore discriminante per identificare le varie categorie sulla base dell’architettura hardware necessaria e risulta ridotta se paragonata a PC o simili. È possibile infatti fornire una prima suddivisione sulla base del tipo di elaboratore utilizzato in tre macro-gruppi:

- PLC (Programmable Logic Controller)
- Microcontrollori
- SoC (System-on-Chip)

Generalmente hanno buoni consumi energetici e dimensioni compatte. Infatti di per sé non producono abbastanza energia per alimentare attuatori esterni ( ma possono attuare degli apparati che fanno funzioare apparati più grossi ).
Un esempio: un Espressif esp8266, circa un pollice di grandezza, in ingresso accetta 3.3V/5V, in uscita dai singoli gpio 20/30 milliAmpere (in totale non più di 50/60 milliAmpere).

### Descrivere PLC

I PLC (o Programmable Logic Controller) sono dispositivi relativamente semplici, progettati per sistemi di automazione e programmati per eseguire funzioni specifiche (per questo non sono prodotti su larga scala). In molti casi vengono programmati individualmente per svolgere una sola funzione specifica, in cui anche il dispositivo hardware é progettato per determinate condizioni di lavoro (come l'ambiente in cui verrà utilizzato). Questo non li rende interscambiabili o compatibili tra loro (a volte anche tra lo stesso produttore).

È dotato di una buona connettività input/output per interagire con vari sensori (temperatura, pressione, peso, posizionamento ecc) ed attuatori (relé, motori elettrici o uscite analogiche).
Sono programmabili con 4 linguaggi:
...
### Descrivere MCU ed elencare i suoi componenti interni

Un microcontrollore o MCU (MicroController Unit) consiste in un sistema hardware completo in un singolo chip (all'inizio venivano chiamati Computer On Chip). Inziano ad essere sviluppati nei primi anni 70 e attualmente, viste le loro proprietà (efficenti, piccoli, veloci) vengono forniti molti ambienti di sviluppo, sia a basso che alto livello, specifici per le singole piattaforme.

Hanno una potenza di calcolo relativamente limitata (dai 200 Mhz in giù, spesso sotto i 100 Mhz), ma possiedono ottimi valori di consumi energetici (meno di 1 W) e ottima efficienza termica. Per fare un confronto con una CPU desktop consumer attuale la sua velocità può arrivare fino a 4 Ghz a fronte di un consumo variabile da 30 a 100 W in base all'uso, e una scarsa efficenza termina.
Va considerato anche che il prezzo di produzione e vendita di un MCU é molto basso. Queste caratteristiche hanno deterinato il suo enorme successo in ambito embedded, sopratutto per sistemi che svolgono operazioni semplici (come misurazione dati, sistemi di controllo, regolazori) in vari ambiti sparsi tra automotive, privato ed industriale.

La sua dotazione di componenti:
...
- strumenti di debug e di scrittura di memoria interna (JTAG)
- nei modelli avanzati anche Wifi, ZigBee, Ethernet, Touch Screen, LCD, ecc..
...
Può contenere: CPU, GPU, ISP (Image Signal Processor), Decoder/Encoder video, WiFi, Bluetooth, Display controller, Audio Controller, seriali (SPI, I2C, I2S), USB, ADC/DAC, Sensori, GPS, Modem, RAM (sporadica), FLASH (molto raro). È improbabile trovarli tutti in un singolo SoC, tendenzialmente si trova un sottoinsieme.

Sono usati praticamente ovunque: automotive, militare, domotica, Set Top Box (lettori multimediali, sistemi tipo chrome cast ecc), navigatori satellitari e telefonia mobile. Al giorno d'oggi la differenza tra SoC e MCU si è fatta più sottile, tanto che in alcuni contesti il termine é utilizzato in modo equivalente per dispositivi che dispongono di una architettura da MCU ma di una dotazione da SoC.

Dagli anni 80 in poi ARM e MIPS sono state le due architetture di riferimento in questo settore, anche se ne esistono altre.

## Concetti

### Sistemi monoprogrammati e multiprogrammati, differenze, caratteristiche, cosa c'entra la MdT?

Questi due termini vengono si riferiscono ad ambienti opertativi che supportano l'eseguzione "contemporanea" di un solo programma o di più programmi.

In un sistema monoprogrammato è possibile eseguire un solo programma alla volta senza parallelismi (un solo thread). Anche se il programma può contenere subcomportamenti, quali funzioni o subroutine che vengono attivati in base a condizioni, le istruzioni vengono eseguite una ad una, senza nessun tipo di parallelismo.
Ergo: se è stata attivata una subroutine il resto del programma é disattivo finchè tale subroutine non resistuisce il controllo alla routine chimante.

Un sistema multiprogrammato può essere dotato di più MdT (CPU o core) che funzionano in parallelo, anche sullo stesso nastro se si implementano sistemi di mutua esclusione. E' quindi possibili eseguire più programmi "contemporaneamente" (o con il time sharing).

Un sistema monoprogrammato con architettura semplice che si può immaginare per un computer è una Macchina di Turing (MdT). E' una macchina a stati, cioè un meccanismo capace di reagire al variare di un input, azionando un meccaniscmo interno in baso al dato in ingresso e il suo stato interno in cui si trova, dotata di più testine/cursori che leggono e scrivono simboli su uno o più nastri.
Esegue constantemente in loop le seguenti azioni: legge il simbolo sotto il cursore, interpreta il simbolo secondo la macchina a stati interna, sposta il cursore, opzionalmente scrive un simbolo sul nastro (prima o dopo lo spostamento).

È possibile effettuare un paragone con la MdT e un computer, paragonando la macchina a stati alla cpu, il nastro alla memoria e l'alfabeto dei simboli sul nastro rappresentato dai numeri binari e la lunghezza dei movimenti del nastro come la grandezza massima della memoria.

### Conversione AD e DA, caratteristiche, pregi e difetti, considerazioni sul flusso di dati campionati

La conversione AD (Analogico-Digitale) trasforma un segnale analogico che varia nel tempo con continuità in un uno digitale, effettuando una serie discreta di misurazioni. Il segnale in ingresso viene **campionato**, cioè misurato in tempi regolari, e produce una sequenza di misure. La velocità con cui si fanno le misure è indicata con la **frequenza di campionamento** e il tempo passato tra le misurazioni è chiamato **tempo di campionamento**. Si otteranno quindi misurazioni tanto più fedeli al segnale in ingresso tanto quanto è la sua frequenza di campionamento.

La conversoine DA (Digitale-Analogica) è il processo inverso, si genererà quindi un segnale analogico continuo in a partire da una sequenza di misure, tramite dei generatori programmabili di tensione.
...
Per Multiplexing si intendono le metodologie per condividere un canale per trasportare più flussi informativi contemporaneamente. Esistono due principali modalità di implementazione:

- **divisione di frequenza**: i segnali usano frequenze diverse e il ricevente si sintonizza su quella desiderata (es. frequenze telefoni, radio). Se i segnali sono "lontani" di frequenza tra di loro i ricevitori riescono ad ascoltare solo quello selezionato. Al contrario se sono troppo vicino c'è interferenza.
- **divisioni dei tempo**: i segnali vengono inviati sul canale in alternanza (più o meno veloce), il ricevente (de-multiplexer) smista i segnali (es. avvisi con altoparlanti, i messaggi sono letti in sequenza preceduti dal nome).

Vengono usati per esempio per "moltiplicare" un ingresso digitale/analogico per leggere le informazion da più sensori senza dover dedicare un pin ognuno.
...
Esempio: ipotizzando che vogliamo un motore che vada ad un regime di 1400 giri
...
Esempio: se necessito di un numero che va da 1 a 10 è inutile dichiarare una variabile di tipo long(4B) e neanche un int(2B), ma basterebbe un byte(1B # da 0 a 255) o un char(1B).  
...
Ciò vale per qualsiasi stringa predefinita. La stringa verrà presa direttamente dalla flash quando necessaria anziché ricopiarla prima in RAM.
...
Si aggancia una funzione ISR ad un segnale:
...
- la libertà di condividere il software con gli amici e i vicini, etc ...
...
### Descrivere a grandi linee il funzionamento di un resistore/condensatore/transistor/etc./... e citarne gli utilizzi
...
### Descrivere un filtro passa alto/basso ed elencare gli utilizzi
...
Ad esempio: un condensatore scarico impiegherà t tempo per caricare il 63,2% della sua capacità totale, un altro t tempo per caricare il 63,2% del rimanente e così via per avvicinamenti successivi fino al limite del 100%.
...
### Descrivere le leggi di Ohm e loro significato
...
### Panoramica generale su sensori "fisici" ed elencare alcuni esempi
...
### Panoramica generale su sensori "numerici" ed elencare alcuni esempi
...
### Panoramica generale su attuatori ed elencare alcuni esempi

Un attuatore è un oggetto che cambia le sue proprie proprietà meccanica al variare dei segnali elettrici che gli vengono forniti.
Ad esempio, il relè è un attuatore, anche se particolare, perchè varie le sue proprietà elettriche (passando per quelle meccaniche di un elettromagnete) in funzione della corrente che vi scorre.
Anche un semplice motore a corrente continua è un attuatore: passa da uno stato statico ad in moto in funzione della tensione/corrente.
Un servomotore o motore passo-passo sono motori particolari che in funzione del segnale in ingresso posizionano il loro asse secondo un angolo preciso.

### Panoramica generale su motori ed elencare alcuni esempi

- **motore a corrente continua**: passa da statico a in movimento in base alla tensione o corrente applicata negli avvolgimenti, permettendo di girare
...
- **ohmometro**: misura i valori di resistenza e lo si può usare per valutare la condizione di una giunzione (diodi e transistor). E' uno strumento attivo composto da un amperometro e una pila che fornisce tensione. Viene immessa la tensione di regime al componente da valutare e misurando la corrente (intensità) che scorre nel circuito e usato la legge di Ohm $R_{incognita}=\frac{V_{immessa}}{I_{misurata}}$. Le giunzioni possono essere:
...
Per definire un'architettura è essenziale conoscere il suo modello astratto, denominato Instruction Set Architecture (ISA). Questo modello include la tipologia di dati trattabili, i registri, la modellazione di input/output e le istruzioni macchina. Da un singolo ISA possono derivare diverse implementazioni (processori anche di anni diversi possono condividere lo stesso ISA).
...
Le MCU AVR sono state le prime ad utilizzare una memoria Flash On-Chip anziché ROM/EEPROM esterne. Sono utilizzate nelle applicazioni embedded a bassa richiesta computazionale, Sono diventate più popolari con la nascita di Arduino che le implementava (casa madre delle AVR: **Atmel**). L'architettura AVR32 è stata progetta per entrare in competizione con ARM per gli embedded di fascia media. E' suddivia in 4 famiglie principali: *tinyAVR*, *megaAVR*, *XMEGA* e *FPSILC*.

### Descrivere architettura Xtensa con esempi

Un microcontrollore RISC ad elevate prestazioni con bassi consumi. L'architettura puntava sulla compatezza, estrema flessibilità ed estensibilità. Il suo isa, nonostante a 32 bit, era capace di eseguire istruzioni a 16 o 24 bit riducendo così le dimensioni del codice. Può essere usato sia in Big che Little endian. Ha trovato spazio nel mercato DSP legato all'audio (AMD TrueAudio) e nei chip Serial-WiFi, come ad esempio quelli usati dalla famiglia Esp8266.

### Cosa si intende con SoC/demo board/eval board/validation board?

- **evaluation board** (o demo board): un PCB di piccole/medie dimensioni con il chip di riferimento e i collegamenti fisici per ad altri dispositivi hardware (bus, video, rete, video, JTAG (per il debug) ) al fine di facilitare lo sviluppo hd e sw di prodotti basati sul chip e per mostrare le sue potenzialità (demo board). Dispone di tutti (o quasi) i collegamenti fisici per l'hardware presente nel MCU o SoC e dispositivi per mostrare le potenzialità. Ad esempio, la RaspberryPI ha avuto successo tramite la sua eval board.
- **validation board**: board di grandi dimensioni con lo scopo di effettuare "silicon validation", ovvero una verifica per evidenziare la prenseza o meno di errori di progettazione o di strutturazione all'interno del chip. Sono munite di numerose porte di debug e tanti pin e test point.

### Storia e architettura Arduino

Arduino nasce in Italia come dispositivo di prototipazione a basso costo per fini didattici. Tra le sue caratteristiche più apprezzate è la sua possibilità di disporre di tutti i progetti hardware rilasciati in modalità open e sul mercato. Sono approdati molti kit per la personalizzazione. Arduino è diventato molto noto nel mercato amatoriale. Oltre al dispositivo è presente un IDE che rendeva molto semplice la scrittura e installazione di software sulla board (chiamati sketch).

### Descrivere piattaforma STM32

MCU con architetture semplici, basati su famiglia ARM (profilo M), facilmente personalizzabili e con la possibilità di lavorare facilmente con sistemi senza OS. Sono utilizzate in campi dove è richiesto di superare rigidi protocolli di certificazione come l'ambito medico, automotive e militare. Esistono diversi modelli differenziati da hardware e dotazioni di base. Esistono porting di sistemi come Linux ma non rendono questa alternativa ancora rapida. Date le dimensioni ridotte di Static RAM e della Flash si prediligono sistemi minimali rispetto ad un sistema operativo completo, massimizandone l'efficienza computazionale.
...
É molto usato in ambito nautico e per il GPS.
...
Il meccanisco di boot é il seguente:
...
Funzione **setup()**: messe tutte le istruzioni di configurazione iniziale (ad es. l’apertura della seriale, la richiesta di indirizzo IP, etc.) da fare una volta per tutte.

Funzione **loop()**: invece tutte le istruzioni vere e proprie del programma che si vuol produrre. Viene chiamata a ripetizione.
...
Pin **digitali**: accettano 2 valori: 1 HIGH, 0 LOW  
Pin **analogici**: gestiscono diversi livelli di tensione (256 o 1024) tra 0V e 5V
...
Esistono molti protocolli a livello applicativo che possono scambiare dati anche nel contesto embedded. Sono più diffusi quelli improntati alla semplicità di implementazione e all'efficenza di trasmissione, fattori essenziali per dispositivi con potenze di calcolo e memoria non disponibili in grandi quantità.

Il protocollo di comunicazione IOT più diffuso è **MQTT** (Message Queuing Telemetry Transport). E' un protocollo che si basa sul patter publisher/subscriver (noto anche come observer/observee) dove i messaggi vengono scambiati tra i clienti verso un dispatching broker che li redistribuisce a tutti i nodi che sono sottoscritti a quel dato argomento di interesse. Ogni messaggio MQTT è coposto da:

- **topic**: string di parole separate da "/", viene usato per identificare l'argomento di cui fa parte il messaggio
- **payload**: il contenuto del messaggio (stringa UTF-8)

Grazie alle sue proprietà ogni dispositivo riceverà solamente i messaggi dei topic a cui è iscritto, senza vedere tutti i messaggi condivisi nella rete, alleggerendo il carico e consumo di risorse.

Nella maggior parte delle implementazione le due principali funzioni sono:

`Subscribe(String subTopic, Function callback)`

`Publish(String topic, String message)`

### Citare protocolli wireless utilizzati in ambiti iot

Due standard molti noti sono **LoRa** e **LoRaWAN**. Lo standard **LoRa** (LOng RAnge) si riferisce a livello fisico (PHY) e definisce le modalità di trasmissione (modulazione) e le frequenze utilizzate (169, 433, 868 e 915 Mhz a seconda dei paesi). La modulazione è utilizzata è la cosidetta *chirp spread spectrum* che sfrutta un segnale a frequenza variabile (sweep) per la codifica. Lo standard **LoRaWAN** (LOng RAnge Wild Area Network) definisce lo strato MAC cioè il protocollo vero e proprio di trasmissione, indirizzamento, crittografia, formato dati, ... . Queste soluzioni sostituiscono o affiancano gli standard WiFi (IEEE 802.11x) per supportare comunicazione tra sensori e sensori gateway. Lo standard mira anche a sostituire quello iFi (che instradano verso nodi terminali o verso reti tradizionali) in contesti a basso consumo e lunghe portare.
I loro vantaggi vantaggi principali sono:

- comunicazioni bidirezionali
- buona robustezza alle interferenze
- consumi ridotti
- lungo raggio (anche nell'ordine dei Km)
...
- Classe B: nodi bidirezionali come A in cui si possono anche definire finestre temporali di ricezione (necessitano sincronizzazione tra sorgente e ricevente)
...
- **canale trasmissivo**: mezzo utilizzato per la comunicazione (es. Wireless, Cavo UTP, Linea Telefonica, Fibra Ottica, Satellitari, Reti Elettriche)
